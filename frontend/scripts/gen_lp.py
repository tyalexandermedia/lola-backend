...
* Pricing is the two-tier monthly model: $697 (Growth, floor) / $997 (Pro).
...
# Two-tier monthly pricing (Starter retired 2026-06-24; old $47/$397/$6,970 dead).
TIERS = [
    {"name": "Growth", "price": "$697/mo", "featured": True,
     "blurb": "Everything Lola tracks — GBP + Map Pack, citations, content, review velocity, call tracking, AI-search visibility. No feature gates. Most popular."},
    {"name": "Pro", "price": "$997/mo", "featured": False,
     "blurb": "Everything in Growth + multi-location / service-area pages, a monthly strategy call with Coach Ty, and priority support."},
]
PRICE_RANGE = "$697-$997"
LOW_PRICE = "697"
HIGH_PRICE = "997"
...
def render_page(svc_slug, svc, city_slug, city):
    ...
    desc = (f'Done-for-you local SEO for {cname} {svc["noun"]}. Rank on Google + '
            f'ChatGPT for high-intent searches. Two monthly plans from $697/mo. '
...
def render_vercel(slugs):
    ...
    # 3. Backend proxies — reviews, tracking (/t/*), reporting, lead-gen,
    #    and admin all live on Railway. ORDER matters (first match wins),
    #    so these must precede the catch-all below.
    ...
    rewrites.append({"source": "/reporting/(.*)",
                     "destination": "https://lola-backend-production.up.railway.app/reporting/$1"})
    rewrites.append({"source": "/lead-gen/(.*)",
                     "destination": "https://lola-backend-production.up.railway.app/lead-gen/$1"})
    rewrites.append({"source": "/admin/(.*)",
                     "destination": "https://lola-backend-production.up.railway.app/admin/$1"})
    # 4. Catch-all LAST.
