  ...
  return (
    <div className={className}>
      ...
      <div className="relative hidden sm:block">
        ...
        <div
          aria-hidden
          className="absolute left-[16.66%] right-[16.66%] top-[26px] h-[2px] bg-gradient-to-r from-gold/20 via-gold/60 to-gold/20"
        />
              ...
              <span
                className={`relative z-10 flex h-[54px] w-[54px] items-center justify-center rounded-full border text-[22px] ${
                  s.free
                    ...
                    : 'border-gold bg-gradient-to-br from-[#1A1408] to-surface shadow-[0_0_24px_rgba(212,175,55,0.25)]'
                }`}
              >
                {s.icon}
              </span>
              <div className="mt-4 flex h-full w-full flex-col rounded-[14px] border border-white/[0.08] bg-white/[0.02] p-5 transition-all duration-300 hover:-translate-y-1 hover:border-gold/40">
                <p className="text-[10px] font-bold uppercase tracking-[0.18em] text-gold/80">{s.kicker}</p>
                <p className="mt-2 text-[17px] font-bold text-white">{s.name}</p>
                <p className="mt-1 text-[20px] font-extrabold text-gold">
                  {s.price}
                  {!s.free && <span className="text-[11px] font-medium text-ink-3"> {s.period}</span>}
                </p>
                <p className="mt-3 flex-1 text-[13px] leading-[1.55] text-ink-2">{s.blurb}</p>
                <a
                  href={s.href}
                  className="mt-4 inline-flex items-center justify-center gap-1 text-[12px] font-bold uppercase tracking-[0.05em] text-gold transition hover:text-gold-bright"
                >
      ...
      <ol className="relative sm:hidden">
        ...
        <div
          aria-hidden
          className="absolute bottom-6 left-[26px] top-6 w-[2px] bg-gradient-to-b from-gold/20 via-gold/60 to-gold/20"
        />
        {STEPS.map((s) => (
          <li key={s.name} className="relative flex gap-4 pb-5 last:pb-0">
            <span
              className={`relative z-10 flex h-[52px] w-[52px] shrink-0 items-center justify-center rounded-full border text-[20px] ${
                s.free
                  ? 'border-gold/50 bg-surface'
                  : 'border-gold bg-gradient-to-br from-[#1A1408] to-surface shadow-[0_0_20px_rgba(212,175,55,0.22)]'
              }`}
            >
              {s.icon}
            </span>
            <div className="flex-1 rounded-[14px] border border-white/[0.08] bg-white/[0.02] p-4">
              <div className="flex items-baseline justify-between gap-2">
                <p className="text-[10px] font-bold uppercase tracking-[0.16em] text-gold/80">{s.kicker}</p>
                <p className="text-[16px] font-extrabold text-gold">
                  {s.price}
                  {!s.free && <span className="text-[10px] font-medium text-ink-3"> {s.period}</span>}
                </p>
              </div>
              <p className="mt-1 text-[16px] font-bold text-white">{s.name}</p>
              <p className="mt-1.5 text-[13px] leading-[1.55] text-ink-2">{s.blurb}</p>
              <a
                href={s.href}
                className="mt-3 inline-flex items-center gap-1 text-[12px] font-bold uppercase tracking-[0.05em] text-gold"
              >
                {s.free ? 'Get my score' : 'Learn more'} →
              </a>
            </div>
