    ...
    <>
      <main className="flex flex-1 flex-col pb-24 sm:pb-12">
        ...
        <section className="relative pt-2 text-center sm:pt-6">
          ...
          <p className="mx-auto mt-6 max-w-[680px] text-[16px] leading-[1.55] text-ink-2 sm:text-[18px]">
          ...
          <div className="mt-7 flex flex-col items-center gap-3 sm:flex-row sm:justify-center">
            <a
              ...
              className="inline-flex min-h-[56px] w-full items-center justify-center gap-2 rounded-[12px] bg-gradient-to-r from-gold via-gold-bright to-gold bg-[length:200%_100%] bg-left px-7 py-3 text-center text-[14px] font-bold uppercase leading-tight tracking-[0.05em] text-on-gold shadow-[inset_0_1px_0_rgba(255,255,255,0.3),0_6px_20px_rgba(212,175,55,0.32)] transition-all hover:bg-right hover:shadow-[inset_0_1px_0_rgba(255,255,255,0.4),0_10px_32px_rgba(212,175,55,0.55)] sm:h-16 sm:w-auto sm:text-[15px]"
            >
              🦴 Start my build →
            </a>
            ...
            <a
              ...
              className="hidden sm:inline-flex h-16 items-center justify-center rounded-[12px] border border-white/[0.15] bg-white/[0.02] px-7 text-[15px] font-semibold uppercase tracking-[0.05em] text-gold transition-all hover:border-gold/40 hover:bg-gold/[0.06]"
            >
            ...
            <a
              ...
              className="text-[13px] font-semibold text-gold underline-offset-2 hover:underline sm:hidden"
            >
        ...
        <section className="mt-16 sm:mt-20">
          ...
          <p className="mt-3 max-w-[640px] text-[14px] leading-[1.6] text-ink-2 sm:text-[15px]">
          ...
          <div className="mt-8 grid grid-cols-1 gap-8 lg:grid-cols-[1fr_320px] lg:items-start">
            ...
            <ul className="flex flex-col gap-3">
              {BUILD.includes.map((item) => (
                ...
                >
                  <span aria-hidden className="mt-0.5 text-[20px] text-gold">✓</span>
            ...
            <div className="rounded-2xl border-[1.5px] border-gold/50 bg-gradient-to-br from-[#1A1408] via-surface to-on-gold p-7 text-center shadow-[0_0_28px_rgba(212,175,55,0.14)] sm:p-8">
              ...
              <p className="mt-3">
                <span className="inline-block bg-gradient-to-br from-gold-hi via-gold-bright to-gold bg-clip-text text-[52px] font-extrabold leading-none text-transparent sm:text-[60px]">
                  {BUILD.price}
                </span>
              </p>
              <p className="mt-2 text-[13px] font-semibold text-ink-3">one-time · no contract</p>
              <a
                href={heroHref}
                target="_blank"
                rel="noopener noreferrer"
                onClick={() => track('retainer_cta_clicked', { from: 'price_card' })}
                className="mt-6 inline-flex min-h-[52px] w-full items-center justify-center gap-2 rounded-[12px] bg-gradient-to-r from-gold via-gold-bright to-gold bg-[length:200%_100%] bg-left px-5 text-[13px] font-bold uppercase tracking-[0.05em] text-on-gold shadow-[inset_0_1px_0_rgba(255,255,255,0.3),0_6px_20px_rgba(212,175,55,0.32)] transition-all hover:bg-right"
              >
                Start my build →
              </a>
              <p className="mt-3 text-[11px] text-ink-3">{buildPay ? '🔒 Secure checkout · one-time $397/month · 90-Day Promise' : '🔒 No payment to book a call'}</p>
            </div>
          </div>
        </section>
        ...
        <section className="mt-16 sm:mt-20">
          <div className="relative overflow-hidden rounded-2xl border-[1.5px] border-gold/50 bg-on-gold p-7 shadow-[0_0_44px_rgba(212,175,55,0.12)] sm:p-9">
            ...
            />
            <p className="relative flex items-center gap-2 text-[12px] font-bold uppercase tracking-[0.28em] text-gold">
        ...
        <section className="mt-16 sm:mt-20">
          ...
          <div className="mt-8 grid grid-cols-1 gap-4 sm:grid-cols-3">
            {[
              ...
              <div key={x.n} className="rounded-[12px] border border-gold/20 bg-white/[0.02] p-5 sm:p-6">
                <p className="text-[12px] font-bold uppercase tracking-[0.22em] text-gold">Step {x.n}</p>
                <p className="mt-2 text-[17px] font-bold text-white">{x.h}</p>
                <p className="mt-1.5 text-[13px] leading-[1.55] text-ink-2">{x.s}</p>
              </div>
            ))}
          </div>
        </section>
        ...
        <section className="mt-16 sm:mt-20">
          ...
          <ul className="mt-8 flex flex-col gap-3 text-[15px] leading-[1.55] text-white sm:text-[16px]">
            {[
              ...
              <li
                key={point}
                className="flex items-start gap-3 rounded-[12px] border border-gold/15 bg-white/[0.02] px-5 py-3"
              >
                <span aria-hidden className="mt-0.5 text-[18px] text-gold">✓</span>
                <span>{point}</span>
              </li>
            ))}
          </ul>
            ...
            {BUILD_FAQS.map((item, i) => (
              ...
              >
                <summary className="flex cursor-pointer list-none items-center justify-between gap-3 p-5 text-[15px] font-semibold text-white sm:p-6 sm:text-[16px] [&::-webkit-details-marker]:hidden">
                  ...
                  <span
                    aria-hidden
                    className="shrink-0 text-[18px] text-gold transition-transform group-open:rotate-45"
                  >
                    +
                  </span>
                </summary>
                <p className="px-5 pb-5 text-[14px] leading-[1.65] text-ink-2 sm:px-6 sm:pb-6 sm:text-[15px]">
                  {item.a}
                </p>
              </details>
            ))}
        ...
        <section className="mt-16 text-left sm:mt-20">
          ...
          <div className="mt-6 grid grid-cols-1 gap-8 sm:grid-cols-[220px_1fr] sm:items-start sm:gap-10">
            <figure className="mx-auto w-[220px] max-w-full sm:mx-0">
              <div className="aspect-[3/4] overflow-hidden rounded-2xl border border-gold/30 shadow-[0_8px_32px_rgba(0,0,0,0.45)]">
                <img
              ...
              <figcaption className="mt-2 text-center text-[11px] text-ink-3 sm:text-left">
                Ty &amp; Lola · St. Pete, FL
              </figcaption>
            </figure>

            <div className="space-y-4 text-[15px] leading-[1.65] text-ink-2 sm:text-[16px]">
              <p>
              ...
              <p>
                ...
                <a href="/r/client/sandbar" className="font-semibold text-gold underline-offset-2 hover:underline">
              ...
              <p className="text-white">
                ...
                <span className="font-bold text-gold">the backyard she deserves</span>. She's
              ...
              <div className="border-t border-white/10 pt-5">
                <p className="text-[18px] text-gold">— {FOUNDER.knownAs}</p>
        ...
        <section className="mt-16 rounded-3xl border border-gold/40 bg-gradient-to-br from-gold/[0.10] via-gold-deep/[0.05] to-on-gold p-7 text-center shadow-[0_0_44px_rgba(212,175,55,0.15)] sm:mt-20 sm:p-12">
          ...
          <a
            ...
            className="mt-7 inline-flex min-h-[56px] w-full max-w-[420px] items-center justify-center gap-2 rounded-[12px] bg-gradient-to-r from-gold via-gold-bright to-gold bg-[length:200%_100%] bg-left px-8 py-3 text-center text-[14px] font-bold uppercase leading-tight tracking-[0.05em] text-on-gold shadow-[inset_0_1px_0_rgba(255,255,255,0.3),0_6px_20px_rgba(212,175,55,0.32)] transition-all hover:bg-right hover:shadow-[inset_0_1px_0_rgba(255,255,255,0.4),0_10px_32px_rgba(212,175,55,0.55)] sm:h-16 sm:w-auto sm:max-w-none sm:text-[16px]"
          >
            🦴 Start my build →
          </a>

          <p className="mt-4 text-[13px] text-gold/85">
            <a
              href="/apply"
          ...
          <p className="mt-5 flex flex-wrap items-center justify-center gap-x-3 gap-y-1 text-[11px] text-ink-3 sm:text-[12px]">
      ...
      <div className="fixed inset-x-0 bottom-0 z-50 border-t border-gold/30 bg-on-gold/95 px-4 py-3 shadow-[0_-8px_24px_rgba(0,0,0,0.5)] backdrop-blur-[12px] sm:hidden">
        <a
          ...
          className="flex min-h-[48px] w-full items-center justify-center gap-2 rounded-[10px] bg-gradient-to-r from-gold via-gold-bright to-gold px-4 text-center text-[13px] font-bold uppercase leading-tight tracking-[0.04em] text-on-gold shadow-[0_4px_16px_rgba(212,175,55,0.4)]"
        >
