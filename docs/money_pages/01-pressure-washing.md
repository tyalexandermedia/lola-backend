# 01 — Pressure Washing (Florida)

## Wix page settings
- **Slug**: `/local-seo-pressure-washing-florida`
- **Title tag** (under 60): `Local SEO for Pressure Washing in Florida | Ty Alexander Media`
- **Meta description** (under 155): `Done-for-you local SEO for Florida pressure washing companies. Free Growth Score, then DIY ($197) or Full Build ($997). Get found on Google + AI search. No contracts.`
- **OG title / Twitter**: same as title tag
- **OG description**: same as meta description
- **OG URL**: `https://tyalexandermedia.com/local-seo-pressure-washing-florida`
- **Canonical**: same as OG URL

## JSON-LD (paste in Wix → Advanced SEO → Structured Data)

```json
{
  "@context": "https://schema.org",
  "@type": "Service",
  "serviceType": "Local SEO for Pressure Washing Companies",
  "provider": {
    "@type": "LocalBusiness",
    "name": "Ty Alexander Media",
    "url": "https://tyalexandermedia.com",
    "telephone": "+1-XXX-XXX-XXXX",
    "address": {
      "@type": "PostalAddress",
      "addressLocality": "Tampa",
      "addressRegion": "FL",
      "addressCountry": "US"
    },
    "areaServed": "Florida"
  },
  "areaServed": "Florida",
  "offers": {
    "@type": "AggregateOffer",
    "lowPrice": "197",
    "highPrice": "997",
    "priceCurrency": "USD",
    "offerCount": "2"
  }
}
```

---

## Page content

### H1
**Florida Pressure Washing Companies: Stop Losing Jobs to Better-Ranked Competitors.**

### Subhead
While you're answering the phone and quoting jobs, your competitors are ranking #1-3 on Google and ChatGPT — and taking the calls you should be getting. We build it. We rank it. Full Build is a one-time $997, backed by our Half-Back Guarantee.

### Primary CTA (above the fold)
**[Get Your Free Growth Score →]** → `https://lola.tyalexandermedia.com/audit?utm_source=tam&utm_medium=landing&utm_campaign=pressure-washing-fl&trade=Soft%20Wash`

60-second scan · delivered by text + email within 24 hours.

---

### NOT THIS / THIS

**NOT THIS:**
- 12-month SEO contracts at $2,500/mo with no guarantees
- Generic agencies who've never washed a driveway
- 50-page reports that go in a folder and die there
- Tools that tell you what's broken but never fix it

**THIS:**
- One-time **$997 Full Build** — a new site, built and ranked. No monthly contract.
- Built by contractors, for contractors (we know your dad's business)
- 30 days getting you found on Google AND the AI answer engines (ChatGPT, Perplexity, Gemini) — with direct access to Ty during the build
- **Half-Back Guarantee**: at least 1 of your 5 money keywords on page 1 or in the map pack in 30 days, or half back

---

### What we do (for pressure washing in Florida)

1. **GMB optimization** — service area tuning, photo updates, post cadence
2. **Citation cleanup** — fix NAP inconsistencies across 30+ directories that hurt your map pack ranking
3. **AI Search Visibility tracking** — monitor where you show up when someone asks ChatGPT "best soft wash in [your city]"
4. **Content built for soft wash buyers** — service pages for house wash, roof cleaning, paver sealing, fleet washing
5. **Review velocity engine** — automated requests post-job, branded review responses
6. **Local link building** — partnerships with HOAs, real estate agents, property managers
7. **Schema markup** — LocalBusiness + Service so Google understands you actually exist

---

### Case study

**{{CASE_STUDY}} (Palm Harbor, FL)** — A 15-year family pressure-washing business running the Lola playbook, with every ranking move tracked on a live public dashboard anyone can open — no login, no screenshots. (Results publish there as they're measured; your results will vary.)

---

### Pricing (matches Lola)

Start free: get your **free Growth Score** — a 60-second scan of how you show up on Google and in AI answers, delivered by text + email within 24 hours. Then pick your path:

| Option | Price | What you get |
|---|---|---|
| **DIY** | **$197** one-time | See your score. Fix it yourself. Your full Growth Score + a simple 5-step fix-it checklist. |
| **Full Build** | **$997** one-time | We build it. We rank it — everywhere people search now. |

**Full Build includes:**
- Custom website build
- 30 days of visibility work across Google and AI answer engines (ChatGPT, Perplexity, Gemini)
- Google Business Profile optimization
- Direct access to Ty during the build

**Half-Back Guarantee.** We pick 5 money keywords for your {{VERTICAL}} business together in week 1. If we don't get at least 1 of them ranking on page 1 or in the map pack within 30 days, you get half your investment back. No fine print.

**[Start My Build →](https://lola.tyalexandermedia.com/build?utm_source=tam&utm_medium=landing&utm_campaign=pressure-washing-fl)**

Prefer we just handle it? See the full build. Want to fix it yourself first? Grab the $197 DIY guide.

---

### Coach Ty trust block

Coach Ty runs Ty Alexander Media in Tampa. He built Lola because his dad owns a soft wash business and got tired of watching him lose jobs to bigger companies with worse work but better Google rankings. You don't get a 30-person agency. You get one founder who answers his own phone, does the work with you, and puts a guarantee in writing.

---

### FAQ (5 questions)

**1. Do you only work with Florida pressure washers?**
We're Florida-focused because that's our network and we know the local search landscape. We'll work with pressure washers anywhere with Google Maps — but Florida companies get the deepest local intel.

**2. How long until I see rankings move?**
Half-Back Guarantee: we pick 5 money keywords together in week 1, and get at least 1 on page 1 or in the map pack within 30 days — or you get half your investment back. Full ROI typically compounds from there.

**3. What if you don't rank me?**
You get half back, no argument. We only succeed if you do.

**4. How much of my time does this take?**
A short kickoff to pick your 5 keywords in week 1, plus a quick check-in or two during the build. Everything else we handle.

**5. Why so much less than agencies charging $2,500+?**
Lola is a one-time build — Ty and his team handle the execution a 5-person agency would do, then hand you a site that's built and ranked. You're paying for results, not bloat. See your free Growth Score first, then choose the $197 DIY guide or the $997 Full Build.

---

### Final CTA
**[Get Your Free Growth Score →](https://lola.tyalexandermedia.com/audit?utm_source=tam&utm_medium=landing&utm_campaign=pressure-washing-fl&trade=Soft%20Wash)**

60-second scan · No pricing on the form · See exactly how you show up before you decide.

---

### Internal links (paste in footer block)

- [HVAC contractors Tampa →](/local-seo-hvac-contractors-tampa)
- [Florida roofers →](/local-seo-roofers-florida)
- [Tampa plumbers →](/local-seo-plumbers-tampa)
- [Get your free Growth Score →](https://lola.tyalexandermedia.com)
- [See the full build →](https://lola.tyalexandermedia.com/build)

### External authority links (use 1)

- [Google's Local Search Update guide](https://developers.google.com/search/blog/2024/03/core-update-march-2024)
